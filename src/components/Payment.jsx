...
const checkoutHandler = async () => {
  try {
    ...
    if (!isUserDataValid()) {
      toast.error("Incomplete user information. Please fill all required fields.");
      return;
    }
    ...
    const timeout = setTimeout(() => {
      ...
      setLoading(false);
      toast.error("Payment timeout. Please check your connection and try again.");
    }, 5 * 60 * 1000);
    ...
    const options = {
      ...
      // ✅ No longer calls /paymentverification here
      // ✅ Webhook handles DB update — we just show polling screen
      handler: function (response) {
        clearTimeout(timeout);
        setLoading(false);
        setPaymentProcessing(false);

        // Basic sanity check on Razorpay response
        if (!response?.razorpay_payment_id) {
          toast.error("Invalid payment response. Please contact support.");
          return;
        }

        // ✅ Hand off to PaymentProcessing component
        // It will poll /paymentverification until webhook updates DB
  ...
  <div className="min-h-screen w-full bg-gradient-to-br from-[#fdf5f6] via-white to-[#f5eff0]">
    ...
    <div className="relative py-8 sm:py-12">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* ====== CONTENT AREA — 3 possible states ====== */}
        {processingPaymentId ? (

          // ── STATE 1: Payment done, polling webhook ──────────────────────
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8">
            <PaymentProcessing
              razorpay_payment_id={processingPaymentId}
              onGiveUp={() => {
                setProcessingPaymentId(null);
                toast.info(
                  `Please contact support with Payment ID: ${processingPaymentId}`,
                  { autoClose: false }
                );
              }}
            />
          ...
          // ── STATE 2: Loading amount or processing ───────────────────────
          <div className="flex flex-col items-center justify-center py-16">
            <Spinner />
            <p className="text-gray-600 font-medium mt-4">
              {amountLoading ? "Loading registration amount..." : "Processing payment..."}
            </p>
          </div>
